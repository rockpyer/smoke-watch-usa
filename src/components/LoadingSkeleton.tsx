...
export const ForecastSkeleton = ({ compact = false }: { compact?: boolean }) => (
  <div className={`${compact ? 'p-2 h-16' : 'p-3 h-20'} bg-background/95 backdrop-blur-sm shadow-lg max-w-2xl animate-pulse flex flex-col justify-between`}>
    <div className="flex items-center justify-between">
      <div className={`${compact ? 'h-3' : 'h-4'} bg-gray-300 rounded w-48`}></div>
      <div className="h-6 w-6 bg-gray-300 rounded"></div>
    </div>
    <div className="flex justify-between text-[9px] h-3">
...
export const MapSkeleton = () => (
  <div className="w-full h-full bg-gray-100 rounded-lg animate-pulse flex items-center justify-center relative">
